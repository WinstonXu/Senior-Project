...
def testmodel(file_name):
    ...
    if not os.path.exists(os.path.join(os.path.dirname(__file__), "Uploads", file_name)):
        ...
        pixelData = np.asarray(pixelData, dtype=np.float64).reshape((1, 1,  im.size[0], im.size[1]))
        # pixelData *= 1 / 255.
        newim = Image.fromarray(pixelData[0][0], 'L')
        newim.save("testim.png")
